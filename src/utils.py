...
def get_all_filepaths_in_dir(input_path: Path, pattern = '*') -> list:
    """
    Description
    ...
    # Convert WindowsPath -> String
    file_paths = [str(path) for path in file_paths if path.is_file()]
    
    return file_paths

def get_all_filenames_in_dir(input_path: Path, pattern = '*') -> list:
    """
    Description
    ...
    file_names = [file.stem for file in file_paths]
